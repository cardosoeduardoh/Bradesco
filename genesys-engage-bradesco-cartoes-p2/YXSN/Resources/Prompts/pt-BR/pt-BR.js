...
{
	var numberOriginal = number.toString();
...
//			alert("endDigits" + endDigits + " " + number + " " + eCento);
    ...
    if ( (fractionalPart != undefined) && (fractionalPart != 0) ) {
        ...
        if (numberOriginal.substring(0,2) == "0." || numberOriginal.substring(0,1) == ".") {
			promptsArray.push(promptBaseUrl + "seme_men/f0.vox");
		}
        promptsArray.push(promptBaseUrl + "miscellaneous/point.vox"); 
		var decimal = numberOriginal.split(".")[1];
		if (decimal.substring(0,1) == "0") {
			promptsArray = promptsArray.concat(alphanumericPrompts(decimal));
		} else if (decimal.length == 1) {
			decimal += "0";
			promptsArray = promptsArray.concat(cardinalPrompts(decimal));
		} else {
			promptsArray = promptsArray.concat(cardinalPrompts(fractionalPart));
		}
    }
    return promptsArray;
} 

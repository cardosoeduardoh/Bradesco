...
function getFrasesMenuPosLimite(dados){   
    ...
    var Promocional = dados.UR80.Apl2TCrlim2Promocional;
    var ParsePromocional = parseInt(Promocional.split(/\D+/).join(""), 10) 
    var Promo = parseInt(dados.UR80.AplSin2TCrlim2Promo);
    ...
    __Log('########### Promocional : ' + Promocional);
    __Log('########### ParsePromocional : ' + ParsePromocional);
    ...
    ret["maxTentativas"] = 3//getMaxTentativasMenus(3, parametros);
    ret["frases"] = [];
    ret["fraseNoMatch"] = [];
    ...
    if (dados.parametros['HabOpLimProm2Torres'] && ParsePromocional >0 && Promo == 0 && diferencaData>=0) {
    //if (dados.parametros['HabOpLimProm2Torres'] && parseInt(dados.UR80.Apl2TCrlim2Promocional) >0 && parseInt(dados.UR80.AplSin2TCrlim2Promo) == 0 && diferencaEntreDatas(wNow,dados.UR80.AplDatecrlim2Expir)>=0) {
...
function getFraseRepetirLimite(dados) {
	...
	ret["frases"] = [];
		...
		ret["frases"].push({ "frase" : "ypiiRepetirLimParcTempDig2.wav"});
		ret["frases"].push({ "frase" : "ypiiParaVoltMenuAntDig0.wav"});		
